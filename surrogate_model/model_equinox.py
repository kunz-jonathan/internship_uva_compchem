...
class jax_predictor(eqx.Module):
    ...
    def __init__(self, model_prot, model_pept, key):
        ...
        # output size is 480
        self.pooler_layer_prot = eqx.nn.Linear(
            in_features=640, out_features=640, key=key3
        )
        self.prot_droput = eqx.nn.Dropout(p=0.2)
        self.prot_projection = eqx.nn.Linear(
            in_features=640, out_features=320, key=key1
        )

        self.pooler_layer_pept = eqx.nn.Linear(
            in_features=640, out_features=640, key=key4
        )
        self.pept_droput = eqx.nn.Dropout(p=0.2)
        self.pept_projection = eqx.nn.Linear(
            in_features=640, out_features=320, key=key2
        )
    ...
    def __call__(self, tokens_prot, tokens_pept, key):
        ...
        ### PREDICTION-HEAD ###
        pred_aff = optax.cosine_similarity(
            x_prot, x_pept, axis=0, epsilon=1e-8
        )  # to match the torch implementation
